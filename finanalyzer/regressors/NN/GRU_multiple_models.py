...
from tensorflow.keras.layers import Dense, InputLayer, Flatten, Activation, Reshape, GRU, Dropout
import gc
import tqdm
...
def build_model(input_shape, output_shape, topology):
    ...
    for layer in topology :
        model.add( GRU( units=layer[0], # Dimensionality of the output space
                        activation=layer[1], # Activation function to use
                        recurrent_activation=layer[2], # Activation function for the recurrent step (default: sigmoid)
                        #unroll=True, # Faster but needs more memory (set to False if there is memory issues)
                        return_sequences=True, # Whether to return the last output in the output sequence, or the full sequence
        ))
        # model.add(Dropout(0.2))
        # model.add(Reshape((1, layer[0])))
    # model.add( GRU( units=topology[-1][0], # Dimensionality of the output space
    #                 activation=topology[-1][1], # Activation function to use
    #                 recurrent_activation=topology[-1][2], # Activation function for the recurrent step (default: sigmoid)
    #                 #unroll=True, # Faster but needs more memory (set to False if there is memory issues)
...
def create_name_list(list_topologies):
    ...
    example: [[[70, "relu", "sigmoid"], [50, "relu", "sigmoid"]], ...]
    """
    list_names = []
    for topology in list_topologies :
        name = "GRU_" + str(len(topology)) + "_layers_" + "_".join([str(layer[0]) for layer in topology]) + str(topology[0][1]) + "_activation_" + str(topology[0][2]) + "_recurrentActivation"

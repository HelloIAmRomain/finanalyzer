...
def create_batch_old_v1(points_list, files_obj, length_files, length_input_timeseries, length_output_timeseries, n_inputs, n_outputs):
    """
...
def create_batch_old_v2(points_list, files_array, length_files, length_input_timeseries, length_output_timeseries, n_inputs, n_outputs):
    """
    Create a batch of data
...
for test_bin_path in testFiles :
    ...
    list_points = create_list_possible_points([length_test_file],length_input_timeseries, length_output_timeseries)
    ...
    for m in range(len(models)) :
        ...
        t_test = time.time()
        output_model = np.zeros((batch_size, NARX_feedback_shape, 1))
        
        for coord_batch in tqdm.tqdm(test_dataset) :
            n_batch += 1
            coord_batch = coord_batch.numpy()
            batch_inputs, batch_outputs = create_batch(points_list=coord_batch, files_array=[file_array], length_files=[length_test_file], 
                                                       n_inputs=1, n_outputs=1,length_input_timeseries=length_input_timeseries, 
                                                       length_output_timeseries=length_output_timeseries, lines_keep=list(list_keep.values()))

            
            x_test.append(batch_inputs[0][:,-1].copy())
            y_test.append(batch_outputs[:,-1].copy())
            
            # TODO: if we take more than one output, we need to change the code (keep some of the previous outputs)
            concat = np.concatenate((batch_inputs[0], output_model), axis=1) 

            output_model = model.predict_on_batch(concat)
            y_pred.append(output_model)

            # Convert from 2D to 3D array
            output_model = output_model.reshape((batch_size, NARX_feedback_shape, 1))

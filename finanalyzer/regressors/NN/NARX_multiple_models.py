...
def create_batch(points_list, files_array, length_files, length_input_timeseries, length_output_timeseries, n_inputs, n_outputs, lines_keep):
    ...
    for file_number, line_number in points_list:
        # Read the lines (input)
        inputs = files_array[file_number][line_number:line_number+length_input_timeseries, 0:n_inputs]
        outputs = files_array[file_number][line_number+length_input_timeseries-length_output_timeseries : line_number+length_input_timeseries, n_inputs:]
        # print(inputs.shape, outputs.shape)
        # prev_outputs = files_array[file_number][line_number+length_input_timeseries-prev_values:line_number+length_input_timeseries, n_inputs:]
        for i in range(len(lines_keep)) :
            batch_inputs[i].append(np.concatenate((inputs[lines_keep[i]],outputs[:-1].reshape((length_output_timeseries-1,1)))))
        batch_output.append(outputs[-1])
    
    # add the previous outputs to the input
    # for i in batch_inputs :
    #     i.append(prev_outputs)

    return np.array(batch_inputs), np.array(batch_output)
...
def build_model(input_shape, output_shape, topology):
    """
    Build a NARX to predict a time series from another one
...
output_shape = 1
NARX_feedback_shape = 1
batch_size = 1_024
epochs = 400

models_topology = [[[512,"relu"],[256,"relu"],[128,"relu"],[64,"relu"],[32,"relu"],[16,"relu"],[8,"relu"],[4,"relu"]]
                   ] 
...
shutil.copyfile("NARX_multiple_models.py", path_working_folder+"NARX_multiple_models.py")
...
for topology in models_topology :
    for key in list_keep :
        model = build_model(input_shape=(len(list_keep[key])+NARX_feedback_shape,), output_shape=output_shape, topology=topology)
        models.append(model)
        model_path = path_working_folder + "NARX_" + key + "_" + str(len(topology)) + "layers"
...
for epoch in range(epochs):
    ...
    with tqdm.tqdm(
        ...
        for batch_coords in train_dataset:
            ...
            batch_inputs, batch_outputs = create_batch(points_list=batch_coords, 
                                                       ...
                                                       length_output_timeseries=output_shape + NARX_feedback_shape,
    ...
    for batch_coords in tqdm.tqdm(val_dataset):
        ...
        batch_inputs, batch_outputs = create_batch(points_list=batch_coords, files_array=files_array, 
                                                   ...
                                                   length_input_timeseries=max_shape, length_output_timeseries=output_shape + NARX_feedback_shape,
                                                   lines_keep=list(list_keep.values()))
        for i in range(len(models)) :

...
def create_batch_4(points_list, files_obj, length_files, length_input_timeseries, length_output_timeseries, n_inputs, n_outputs, lines_keep):
    """
    Create a batch of data
    ...
    for file_number, line_number in points_list:
        batch_in = []
        for keep in lines_keep:
            files_obj[file_number].seek(4 * n_inputs * (line_number + keep))
            batch_in.append(np.frombuffer(files_obj[file_number].read(4 * n_inputs), dtype=np.float32))
        batch_input.append(np.array(batch_in))
        # Pointer to the file (output)
        files_obj[file_number].seek(4 * n_inputs * length_files[file_number] + (line_number + length_input_timeseries - length_output_timeseries) * 4 * n_outputs)
        # Read the lines (output)
        batch_output.append(np.frombuffer(files_obj[file_number].read(length_output_timeseries * 4 * n_outputs), dtype=np.float32))
...
for epoch in range(epochs):
    ...
    for batch_coords in tqdm.tqdm(train_dataset):
        ...
        batch_inputs, batch_outputs = create_batch_3(points_list=batch_coords, files_obj=files_obj, 
